...
sub check_type {
  ...
  if($dbname =~ '_core_') {
    if($self->has_type('core')) {
      return {run=>0, 'Test requires a core database'};      
    } elsif(!$dba->isa('Bio::EnsEMBL::DBSQL::DBAdaptor')) {
      return {run=>0, 'Test requires a Bio::EnsEMBL::DBSQL::DBAdaptor'};      
    }
  } elsif($dbname =~ /_otherfeatures_/) {
    if($self->has_type('otherfeatures')) {
      return {run=>0, 'Test requires an otherfeatures database'};      
    } elsif(!$dba->isa('Bio::EnsEMBL::DBSQL::DBAdaptor')) {
      return {run=>0, 'Test requires a Bio::EnsEMBL::DBSQL::DBAdaptor'};      
    }
  } elsif($dbname =~ /_variation_/) {
    if($self->has_type('variation')) {
      return {run=>0, 'Test requires a variation database'};      
    } elsif(!$dba->isa('Bio::EnsEMBL::Variation::DBSQL::DBAdaptor')) {
      return {run=>0, 'Test requires a Bio::EnsEMBL::Variation::DBSQL::DBAdaptor'};      
    }    
  } elsif($dbname =~ /_compara_/) {
    if($self->has_type('compara')) {
      return {run=>0, 'Test requires a compara database'};      
    } elsif(!$dba->isa('Bio::EnsEMBL::Compara::DBSQL::DBAdaptor')) {
      return {run=>0, 'Test requires a Bio::EnsEMBL::Compara::DBSQL::DBAdaptor'};      
    }    
  } else {
