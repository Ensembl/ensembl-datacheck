...
=head2 table_dates
  ...
  Arg [2]    : (optional) name of database
  Example    : my $dates = table_dates($dbc);
  Description: Returns a hashref containing modification dates of all tables 
               for a database to supports test scheduling
  Returntype : Hashref of dates
  Exceptions : croak if DBConnection not passed
  Caller     : general
  Status     : Stable
