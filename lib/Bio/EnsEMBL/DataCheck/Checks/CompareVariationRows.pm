...
package Bio::EnsEMBL::DataCheck::Checks::CompareVariationRows;
...
use constant {
  ...
  DB_TYPES       => ['variation'],
  TABLES         => ['variation', 'variation_feature', 'transcript_variation']
};
...
sub tests {
  ...
  SKIP: {
    ...
    my @tables = ('variation', 'variation_feature', 'transcript_variation');
    
    foreach my $table (@tables) {
      my $desc_1 = "Consistent $table counts between ".
                 $self->dba->dbc->dbname.' and '.$old_dba->dbc->dbname;
      my $sql_1  = "SELECT COUNT(*) FROM $table";
