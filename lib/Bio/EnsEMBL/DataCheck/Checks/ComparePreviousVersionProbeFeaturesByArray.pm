...
package Bio::EnsEMBL::DataCheck::Checks::ComparePreviousVersionProbeFeaturesByArray;
...
use constant {
  ...
  DB_TYPES       => ['funcgen'],
  TABLES         => ['array', 'array_chip', 'probe', 'probe_feature']
};
...
sub tests {
  ...
  SKIP: {
    ...
    my $desc_decrease = "Checking if number of probe features for each array has decreased between ".
                 $self->dba->dbc->dbname.' and '.$previous_dba->dbc->dbname;
    my $desc_increase = "Checking if number of probe features for each array has increased between ".
                 $self->dba->dbc->dbname.' and '.$previous_dba->dbc->dbname;
